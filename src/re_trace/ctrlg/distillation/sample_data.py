...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        tokenizer_name_or_path, padding_side="left"
    )
    ...
    for chunk_idx in range(0, args.total_chunks):
        ...
        for prompt_idx, prompt in enumerate(input_data):
            ...
            chunk_size_per_process_prompt = (
                chunk_size_per_process // len(input_data) + 1
            )
            chunk_size_per_process_prompt = min(
                chunk_size_per_process_prompt,
                chunk_size_per_process - prompt_idx * chunk_size_per_process_prompt,
            )

            if args.cache_prompt_past_key_values and len(prompt_ids) > 1:
                with torch.no_grad():
                    past_key_values = base_model(
                        torch.tensor(prompt_ids[:-1], device=device)
                    ).past_key_values

            for batch_idx in tqdm(
                range(0, chunk_size_per_process_prompt, args.batch_size)
            ):
                batch_size_ = min(
                    args.batch_size, chunk_size_per_process_prompt - batch_idx
                )
                if args.cache_prompt_past_key_values and len(prompt_ids) > 1:
                    model_kwargs = {
                        "past_key_values": tuple(
                            [
                                tuple(
                                    [
                                        col.expand(batch_size_, -1, -1, -1).contiguous()
                                        for col in row
                                    ]
                                )
                                for row in past_key_values
                            ]
                        )
                    }
                else:
                ...
                with torch.no_grad():
                    ...
                    generation_output = base_model.generate(
                        input_ids=torch.tensor(
                            [prompt_ids] * batch_size_, device=device
                        ),
                        pad_token_id=tokenizer.eos_token_id,
                        max_new_tokens=args.max_new_tokens,
                        output_hidden_states=args.save_embeddings,
                        return_dict_in_generate=True,
                        **model_kwargs,
                        **generation_config,
                    )
                    sequences_batch = generation_output.sequences[
                        :, len(prompt_ids) :
                    ].cpu()
                    sequences_batch = pad_to_len(
                        sequences_batch, args.max_new_tokens, tokenizer.eos_token_id
                    )
                    sequences.append(sequences_batch)

                if args.save_embeddings:
                    last_hidden_states = [
                        hs[-1][:, -1, :] for hs in generation_output.hidden_states
                    ]  # gather last Layer hidden_states
                    last_hidden_states = torch.stack(last_hidden_states, dim=1).cpu()
                    last_hidden_states = pad_to_len(
                        last_hidden_states, args.max_new_tokens, tokenizer.eos_token_id
                    )
        ...
        if rank == 0:
            sequences_list = [
                torch.empty_like(sequences, dtype=sequences.dtype)
        ...
        if args.save_embeddings:
            ...
            if rank == 0:
                embeddings_list = [
                    torch.empty_like(embeddings, dtype=embeddings.dtype)
                    for idx in range(world_size)
                ]
        ...
        if rank == 0:
            ...
            output_file = (
                f"{args.output_file}.{chunk_idx}"
                if args.total_chunks > 1
                else f"{args.output_file}"
            )
