...
def Kmeans_faiss(vecs, K, max_iterations=1000, nredo=1, verbose=True):
    kmeans = faiss.Kmeans(
        ...
        nredo=nredo,
        verbose=verbose,
        max_points_per_centroid=vecs.shape[0] // K,
        gpu=True,
    )
    kmeans.train(vecs)
...
def update_flows(
    alpha, beta, gamma, suffixes, idx2cluster, hidden_states, vocab_size, eos_token_id
):
    offset2index = {}
...
def main():
    ...
    suffixes, suffix_embeddings = load_examples(
    ...
    print(
        f"training K-means with {hidden_states-1} clusters and {vecs.shape[0]} suffix embeddings ..."
    )
    kmeans = Kmeans_faiss(
        vecs, hidden_states - 1, max_iterations=args.kmeans_iterations
    )

    print(
        f"clustering {len(suffixes)} suffix embeddings into {hidden_states-1} clusters ..."
    )
