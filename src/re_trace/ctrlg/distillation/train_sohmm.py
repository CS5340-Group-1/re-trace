...
def apply_dropout(input_ids, dropout, vocab_size, eos_token_id):
    ...
    input_ids[
        torch.logical_and(
            torch.rand(n, d, device=input_ids.device) < dropout,
...
def train_hmm(
    ...
    hmm_model = SOHMM.from_pretrained(
        f"{model_path}/checkpoint-{checkpoint}", map_location="cpu"
    ).to(device)
    hidden_states, vocab_size, eos_token_id = (
        hmm_model.hidden_states,
        hmm_model.vocab_size,
        hmm_model.eos_token_id,
    )
    ...
    dev_data = dev_data[
        rank * num_per_process : min(dev_data.shape[0], (rank + 1) * num_per_process)
    ]
    ...
    for step_count, step_size in em_schedule:
        for step_idx in range(0, step_count):
            ...
            train_step = torch.cat(
                [
                    torch.load(f"{data_path}/{dataset}.train.{idx % total_chunks}")
            ...
            train_data = train_step[
                rank
                * num_per_process : min(
                    train_step.shape[0], (rank + 1) * num_per_process
                )
            ]
            ...
            alpha_flow = torch.zeros(
                hidden_states, hidden_states, hidden_states, device=device
            )
            beta_flow = torch.zeros(
                vocab_size + 1, hidden_states, device=device
            )  # (emissions only depend on z_t)
            ...
            with torch.no_grad():
                for batch_idx in tqdm(range(0, train_data.shape[0], batch_size)):
                    ...
                    train_data_batch = train_data[
                        batch_idx : batch_idx + batch_size_
                    ].to(device)

                    probs = hmm_model.forward(train_data_batch)
                    hmm_model.backward(
                        train_data_batch, probs, alpha_flow, beta_flow, gamma_flow
                    )
                ...
                beta_flow[eos_token_id, :] = torch.maximum(
                    beta_flow[eos_token_id, :], eps_cuda
                )

                beta_flow = torch.permute(
                    beta_flow[:-1, :], (1, 0)
                ).contiguous()  # hidden_states * vocab_size
...
if __name__ == "__main__":
    ...
    em_schedule = [
        ...
        for x in args.em_schedule.split(";")
        if x != ""
    ]
