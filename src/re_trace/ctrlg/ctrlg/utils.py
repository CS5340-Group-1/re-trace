...
def distribute_state_weights(E2D, y):
    ...
    return y[E2D[:, None], torch.arange(0, hidden_states, device=device)[None, :]]
...
def aggregate_edge_weights(E2S, y, num_states):
    ...
    y_max = y_out_max[
        E2S[:, None], torch.arange(0, hidden_states, device=device)[None, :]
    ]
...
def ends_at(prefix, suffix, offset_min, D_cache, dfa_model):
    ans = []
...
class ConstraintLogitsProcessor(LogitsProcessor):
    def __init__(
        ...
        T_weights = matmul_a_logb(
            T_mask, torch.transpose(beta, 0, 1)
        )  # num_transitions * hidden_states
        ...
        for t in range(1, max_tokens + 1):
            ...
            y = aggregate_edge_weights(
                E2Src, T_weights + y, num_states=num_states
            )  # num_states * hidden_states
        ...
        C_shape = C.shape
        C = matmul_a_logb(
            range_mask, torch.flatten(C, start_dim=1, end_dim=2)
        )  # num_ranges * (num_states * hidden_states)
    ...
    def __call__(self, input_ids, scores):
        ...
        if len(selected_idx) > 0:
            ...
            hmm_batch_size = (
                len(selected_idx)
                if self.hmm_batch_size is None
                else min(len(selected_idx), self.hmm_batch_size)
            )
            hmm_logits, hmm_logits_ = self.compute_logits(
    ...
    def compute_logits(self, prefixes, token_ranges, batch_size):
        ...
        VE_mask, EV_mask, T_mask = (
            self.dfa_model.VE_mask,
            self.dfa_model.EV_mask,
            self.dfa_model.T_mask,
        )
        A_cache, B_cache, C_cache, D_cache = (
            self.A_cache,
            self.B_cache,
            self.C_cache,
            self.D_cache,
        )
        alpha_exp, beta, gamma = (
            self.hmm_model.alpha_exp,
            self.hmm_model.beta,
            self.hmm_model.gamma,
        )
        hidden_states, vocab_size = (
            self.hmm_model.hidden_states,
            self.hmm_model.vocab_size,
        )
        ...
        if prefix_len > generation_offset:
            # update A_cache
            A = torch.stack(
                [A_cache[prefix[:-1]] for prefix in prefixes], dim=0
            )  # len(prefixes) * hidden_states
            log_probs = torch.stack(
                [beta[:, prefix[-1]] for prefix in prefixes], dim=0
            )  # len(prefixes) * hidden_states
            A += log_probs
            ...
            A = torch.stack(
                [A_cache[prefix] for prefix in prefixes], dim=0
            )  # prefix_num * hidden_states
        ...
        if len(selected_idx) > 0:
            for batch_idx in range(0, selected_num, batch_size):
                ...
                for prefix_idx in selected_batch:
                    ...
                    C_batch.append(
                        C_cache[(remaining_tokens_min - 1, remaining_tokens_max - 1)]
                    )
                C_batch = torch.stack(
                    C_batch, dim=0
                )  # batch_size_ * num_states * hidden_states

                C = (
                    A_batch[:, None, :] + C_batch
                )  # batch_size_ * num_states * hidden_states

                C_shape = C.shape
                C = matmul_log(
                    torch.flatten(C, start_dim=0, end_dim=1), beta
                )  # (batch_size_ * num_states) * vocab_size
                C = C.view(
                    C_shape[0], C_shape[1], -1
                )  # batch_size_ * num_states * vocab_size

                mask = torch.stack(
                    [VE_mask[D_cache[prefix]] for prefix in prefixes_batch], dim=0
                )  # prefix_mask, batch_size_ * num_transitions
                mask = (
                    mask[:, :, None] * EV_mask[None, :, :]
                )  # batch_size_ * num_transitions * num_states
                mask = torch.transpose(
                    mask, 1, 2
                )  # batch_size_ * num_states * num_transitions

                mask_shape = mask.shape
                mask = torch.matmul(
                    torch.flatten(mask, start_dim=0, end_dim=1), T_mask
                )  # (batch_size_ * num_states) * vocab_size
                mask = mask.view(
        ...
        for prefix_idx, prefix in enumerate(prefixes):
            ...
            offsets = ends_at(prefix, suffix, offset_min, D_cache, self.dfa_model)
            for offset in offsets:
                log_prob = logsumexp(A[prefix_idx] + B_cache[offset], dim=0)
                logits[prefix_idx, suffix[offset]] = torch.logaddexp(
                    logits[prefix_idx, suffix[offset]], log_prob
                )
...
def rank_generated_ids(
    base_model,
    generated_ids,
    prompt_ids,
    suffix_ids,
    suffix_logits_only=False,
    suffix_length_cap=None,
    length_penalty=1.0,
):
    ...
    with torch.no_grad():
        ...
        logits = torch.log_softmax(logits, dim=-1)
        log_probs = logits[
            torch.arange(n)[:, None], torch.arange(d - 1)[None, :], input_ids[:, 1:]
        ]

    norms = torch.sum(logits_mask[:, 1:], dim=-1) ** length_penalty
    log_probs = torch.sum(log_probs * logits_mask[:, 1:], dim=-1) / norms

    generated_ids_sorted = [
        a
        for a, b in sorted(
            [(x, y) for x, y in zip(generated_ids, log_probs.tolist())],
            key=lambda x: x[1],
            reverse=True,
        )
    ]
