...
class HMM(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, input_ids):
        ...
        alpha_exp, beta, gamma_exp = (
            self.alpha_exp,
            self.beta,
            torch.softmax(self.gamma, dim=0),
        )
        ...
        input_probs *= (input_ids_ != -1)[:, None, :].expand(
            -1, hidden_states, -1
        )  # 0.0 for MISSING token
    ...
    def backward(self, input_ids, probs, alpha_flow, beta_flow, gamma_flow):
        ...
        input_ids_ = torch.permute(
            input_ids, (1, 0)
        ).contiguous()  # seq_len * batch_size
        input_probs = beta[
            torch.arange(0, hidden_states, device=device)[None, :, None],
            input_ids_[:, None, :],
        ].contiguous()  # seq_len * hidden_states * batch_size
        ...
        for t in range(0, seq_len - 1):
            ...
            pp = (
                probs[layer_idx] - input_probs[t, :, :]
            )  # parent probs; hidden_states * batch_size
        ...
        beta_flow.scatter_add_(
            0, input_ids_, flows.view(seq_len * batch_size, hidden_states)
        )
