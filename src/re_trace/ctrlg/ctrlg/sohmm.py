...
class SOHMM(nn.Module, PyTorchModelHubMixin):
    def __init__(self, hidden_states: int, vocab_size: int, eos_token_id: int):
        ...
        alpha_exp = torch.softmax(
            torch.randn(hidden_states, hidden_states, hidden_states), dim=2
        )
        ...
        gamma = torch.log_softmax(
            torch.randn(hidden_states, hidden_states).flatten(), dim=0
        ).view(hidden_states, hidden_states)
    ...
    def forward(self, input_ids):
        ...
        gamma_exp = torch.softmax(self.gamma.flatten(), dim=0).view(
            self.gamma.shape
        )  # softmax over both dimensions
        ...
        )  # track state pairs (z_{t-1}, z_t, Batch)
        ...
        for t in range(seq_len - 1, -1, -1):
            ...
            y += input_probs[t, :, :].unsqueeze(
                0
            )  # broadcast emissions over the z_{t-1} dimension
            ys.append(y)
