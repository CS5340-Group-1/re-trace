# Picked from https://github.com/yidouweng/trace
import torch
...
def logit_adjustment(
    ...
    log_p_zm_x_less_m = torch.vmap(stable_mvm, in_dims=(None, 0))(
        log_A.squeeze(0).T, log_alpha_prev
    )  # (B, H)

    # Step 2: Compute expected future toxicity for each token
    log_p_x = torch.vmap(stable_mvm, in_dims=(None, 0))(
        log_B.T, log_p_zm_x_less_m
    )  # (B, V)
    log_expectation_zm_x_less_m = log_p_zm_x_less_m + torch.log(
        expectation_zm + epsilon
    )  # (B, H)
    log_expectation_xm = (
        torch.vmap(stable_mvm, in_dims=(None, 0))(log_B.T, log_expectation_zm_x_less_m)
        - log_p_x
    )  # (B, V)
...
class HmmGuidedLogitsProcessor(LogitsProcessor):
    def __init__(
        ...
        if hasattr(hmm_model, "exp_weights_1"):
            ...
            raise AttributeError(
                "HMM model must have either 'exp_weights' or 'exp_weights_1' attribute"
            )
    ...
    def configure_for_prompts(self, prompt_batch: torch.Tensor):
        ...
        if not self.hmm_model or not self.expectation_cache:
            raise RuntimeError(
                "HMM model and expectation cache must be set before configuration."
            )
    ...
    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if not self.is_configured_for_prompt:
                raise RuntimeError(
                    "HmmGuidedLogitsProcessor.configure_for_prompts() must be called before generation."
                )
            ...
            if current_seq_len > expected_seq_len:
                ...
                temp = (
                    self.log_alpha_prev.unsqueeze(2) + self.log_A
                )  # (batch_size, H, H)
                ...
                emission = self.hmm_model.beta[:, new_tokens].transpose(
                    0, 1
                )  # (batch_size, H)
            ...
            expectation_zm = self.expectation_cache[self.generation_step].to(
                self._model_device
            )  # (H,)
