...
def main():
    ...
    parser.add_argument(
        "--hmm_model_path",
        type=str,
        default="models/sohmm_gpt2-large_uncon_seq-len-32_4096_10M",
        help="Path to the trained SOHMM directory",
    )
    parser.add_argument(
        "--prompts_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Strength of SOHMM guidance, often 0~2.0. 0 is no guidance, 1 is default.",
    )
    parser.add_argument(
        "--baseline",
        action="store_true",
        help="Generate baseline (no SOHMM guidance) alongside TRACE",
    )
    parser.add_argument(
    ...
    if args.baseline:
        output_path = os.path.join(
            PROJECT_ROOT, f"results/sohmm_comparison_a{args.a}_generated.csv"
        )
    else:
        output_path = os.path.join(
            PROJECT_ROOT, f"results/sohmm_detox_a{args.a}_generated.csv"
        )
